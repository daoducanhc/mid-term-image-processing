...
for i in range(0,12):
    template = cv2.imread("2/dataset/{}.png".format(i), cv2.COLOR_BGR2GRAY)
    tH, tW = template.shape[:2]
    mask = cv2.imread("2/dataset/{}_mask.png".format(i), cv2.COLOR_BGR2GRAY)
    ...
    _, _, _, max_loc = cv2.minMaxLoc(corrimg)
...
result_temp = cv2.resize(result_temp, (300,600))
